...
export const useShoppinglistDetailsViewStore = defineStore('shoppinglistDetailsViewStore', () => {
  ...
  function setTotalPrice(data: ShoppinglistItemMetadata[]) {
    ...
    data.map(
      (element) =>
        (totalPriceCalculated += element.calculatedPrice !== null ? element.calculatedPrice : 0),
    )
  ...
  function removeShoppinglistItemMetadata(id: number) {
    setShoppinglistDetailsViewItems(
      shoppinglistDetailsViewItems.value.filter((element) => element.id !== id),
    )
  }
